  ...
  switch (pNotification->Custom_Evt_Opcode)
  {
    ...
    case CUSTOM_STM_GOAL_C_WRITE_NO_RESP_EVT:
      /* USER CODE BEGIN CUSTOM_STM_GOAL_C_WRITE_NO_RESP_EVT */
        ...
        {
        	//TODO pending action currently not being sent before set_speed is run, defeating purpose.
        	//Perhaps use sequencer instead for setting speed
        	//P2PS_APP_Motor_Pending_Action(); //TODO check if already in requested state
...
void P2PS_APP_Motor_Pending_Action(void)
{
	Custom_App_Context.State_Status = PENDING;
